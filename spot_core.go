...
func addSpot(name, pass string) error {
	var cmd string
	cmd = fmt.Sprintf(`sudo nmcli con add type wifi ifname "%v" con-name "%v" autoconnect yes ssid "%v"`, wifiInterface, name, name)
	out, err := exe(cmd, "add spot")
	if err != nil {
		return fmt.Errorf("error:%v, out:%v", err.Error(), out)
	}

	cmd = fmt.Sprintf(`sudo nmcli con modify "%v" 802-11-wireless.mode ap 802-11-wireless.band bg ipv4.method shared`, name)
	...
	if pass != "" {
		cmd = fmt.Sprintf(`sudo nmcli con modify "%v" wifi-sec.key-mgmt wpa-psk`, name)
		out, err = exe(cmd, "modify pass type spot")
		if err != nil {
			return fmt.Errorf("error:%v, out:%v", err.Error(), out)
		}

		cmd = fmt.Sprintf(`sudo nmcli con modify "%v" wifi-sec.psk "%v"`, name, pass)
...
func setSpot(mode, name string) error {
	cmd := fmt.Sprintf(`sudo nmcli con "%v" "%v"`, mode, name)
...
func spotExists(name string) (bool, error) {
	cmd := fmt.Sprintf(`sudo nmcli -f NAME con sh`)
